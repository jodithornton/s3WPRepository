...
if ( !function_exists( 'cg_content_nav' ) ) :
	...
	function cg_content_nav( $nav_id ) {
		...
		<nav role="navigation" id="<?php echo esc_attr( $nav_id ); ?>" class="<?php echo esc_attr( $nav_class ); ?>">
			<span class="screen-reader-text"><?php esc_html_e( 'Post navigation', 'broker' ); ?></span>
			...
			<?php elseif ( $wp_query->max_num_pages > 1 && ( is_home() || is_archive() || is_search() ) ) : // navigation links for home, archive, and search pages  ?>
				...
				<?php if ( get_previous_posts_link() ) : ?>
					<div class="nav-next"><?php previous_posts_link( wp_kses( __( 'Newer posts <span class="meta-nav">&rarr;</span>', 'broker' ), $allowed_args )); ?></div>
				<?php endif; ?>
...
if ( !function_exists( 'cg_comment' ) ) :
	...
	if ( !function_exists( 'cg_posted_on' ) ) :
		...
		function cg_posted_on() {
			...
			$posted_on = sprintf(
			esc_html_x( '%s', 'post date', 'broker' ), '' . $time_string . ''
			);

			$byline = sprintf(
			esc_html_x( 'by %s', 'post author', 'broker' ), '<span class="author vcard"><a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">' . esc_html( get_the_author() ) . '</a></span>'
			);
			...
			function cg_get_author_name( $format = 'span' ) {
				...
				if ( $cg_author_name ) {
					if ( $format == 'plain' ) {
						...
						echo '<span class="cg-blog-author author vcard"> ' . wp_kses_post( $cg_author_name ) . ' </span>';
					}
				}
			}
